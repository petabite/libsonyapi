...
class Camera(object):
    ...
    def post_request(self, url, method, param = [] ):
        ...
        if 'error' in list(response.keys()):
            ...
            return response
    # TODO: response handler, return result of set/do etc
    # following functions make call to sony camera api
    def do(self, method):
        """
        use this if no params are needed in api call
        this calls to camera service api, only accepts method arg
        """
        response = self.post_request(self.camera_endpoint_url, method)

    def set(self, method, param):
        """
        use this if method and param are needed in api call
